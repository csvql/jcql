module LexerSpec (testLexer) where
...
testLexer =
  testGroup
    ...
    [ testCase "ignores whitespace" (l " " @?= []),
      ...
      testGroup "lexes basic strings" $ map (\q -> testCase q $ l (q ++ "hello world" ++ q) @?= [TString "hello world" startPn]) lexStringQuotes,
      testGroup "string quotes can be escaped within string" $ map (\q -> testCase (q ++ " can be escaped with \\" ++ q) $ l (q ++ "hello \\" ++ q ++ "world" ++ q) @?= [TString ("hello \\" ++ q ++ "world") startPn]) lexStringQuotes,
      testGroup "lexes operators" $ map (\input -> testCase input (l input @?= [TOperator input startPn])) lexOperators,
      testGroup "lexes brackets" $ map (\input -> testCase input (l input @?= [TBracket input startPn])) lexBrackets,
      testGroup "lexes identifiers" [
        ...
        --  - dot
        --  - dash
        testCase "alphabetic identifier" $ l "quickBrownFox" @?= [TIdentifier "quickBrownFox" startPn],
        testCase "alphanumeric identifier" $ l "a1960b" @?= [TIdentifier "a1960b" startPn],
...
testRandomWordWithRandomCapitalization w a = l w == l randomlyCapitalized
  where
    n = a `mod` length w
    randomlyCapitalized = [if i == n then toLower (w !! i) else toUpper (w !! i) | i <- [0 .. length w -1]]
