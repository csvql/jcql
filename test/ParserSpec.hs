  ...
  testGroup
    ...
    [
        ...
        [
            ...
            testCase "cross join" (parse "import A 'a.csv', B 'b.csv' take A cross join B select a.1" @?= AST [AliasedImport "B" "b.csv", AliasedImport "A" "a.csv"] ("A", [Cross (TableRef "B")], Nothing, [SelectExpr (TableColumn "a" 1)])),
            testCase "inner join with equals" (parse "import A 'a.csv', B 'b.csv' take A inner join B on A.1 = B.1 select a.1" @?= AST [AliasedImport "B" "b.csv", AliasedImport "A" "a.csv"] ("A", [Inner (TableRef "B") (BinaryOpExpr (TableColumn "A" 1) AST.EQ (TableColumn "B" 1))], Nothing, [SelectExpr (TableColumn "a" 1)])),
            testCase "inner join with equals" (parse "import A 'a.csv', B 'b.csv', C 'c.csv' take A inner join B on A.1 = B.1, cross join C select a.1" @?= AST [AliasedImport "C" "c.csv", AliasedImport "B" "b.csv", AliasedImport "A" "a.csv"] ("A", [Cross (TableRef "C"), Inner (TableRef "B") (BinaryOpExpr (TableColumn "A" 1) AST.EQ (TableColumn "B" 1))], Nothing, [SelectExpr (TableColumn "a" 1)]))
        ],
        testGroup "filter" 
        [
            testCase "unary operator" (parse "import A 'a.csv', B 'b.csv' take A cross join B where not 0 select a.1" @?= AST [AliasedImport "B" "b.csv", AliasedImport "A" "a.csv"] ("A", [Cross (TableRef "B")], (Just (UnaryOpExpr NOT (ValueExpr (ValueInt 0)))), [SelectExpr (TableColumn "a" 1)])),
            testCase "binary operator" (parse "import A 'a.csv', B 'b.csv' take A cross join B where a.1 = 0 select a.1" @?= AST [AliasedImport "B" "b.csv", AliasedImport "A" "a.csv"] ("A", [Cross (TableRef "B")], (Just (BinaryOpExpr (TableColumn "a" 1) AST.EQ (ValueExpr (ValueInt 0)))), [SelectExpr (TableColumn "a" 1)])),
            testCase "function" (parse "import A 'a.csv', B 'b.csv' take A cross join B where Coalesce(a.1,a.2) select a.1" @?= AST [AliasedImport "B" "b.csv", AliasedImport "A" "a.csv"] ("A", [Cross (TableRef "B")], (Just (Function "Coalesce" [TableColumn "a" 2,TableColumn "a" 1])), [SelectExpr (TableColumn "a" 1)])),
            testCase "function and unary operator" (parse "import A 'a.csv', B 'b.csv' take A cross join B where Len(a.1) = 0 select a.1" @?= AST [AliasedImport "B" "b.csv", AliasedImport "A" "a.csv"] ("A", [Cross (TableRef "B")], (Just (BinaryOpExpr (Function "Len" [TableColumn "a" 1]) AST.EQ (ValueExpr (ValueInt 0)))), [SelectExpr (TableColumn "a" 1)]))
        ],
